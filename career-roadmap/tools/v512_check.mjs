...
const SITE = 'http://127.0.0.1:8080/career-roadmap/?v512=1';
const results = [];
...
function review(id, start, end, summary, confidence) {
  return {
    ...
    diagnosisUpdates: [{
      ...
      lifecycle: 'supported',
      previousConfidence: confidence - 5,
...
const seedState = {
  ...
  careerModel: [{ diagnosisId: 'diag-test', topic: 'Test Diagnosis', diagnosisConfidence: 40 }],
  diagnosisHistory: [],
  pendingConfidence: [{ diagnosisId: 'diag-test', date: '2026-07-24' }],
...
async function run(type, name, options) {
  ...
  try {
    ...
    await context.addInitScript((state) => {
      localStorage.setItem('jett-career-os-v22', JSON.stringify(state));
    }, seedState);
    ...
    await page.waitForFunction(() => window.CAREER_OS_V512?.complete === true, null, { timeout: 30000 });
    ...
    await page.evaluate(() => openQuick('weekly'));
    ...
    await page.locator('#quickContent [data-save]').click();
    ...
    item.http = response?.status() || 0;
    item.promoted = promoted;
    item.imported = imported;
    item.passed =
      item.http === 200 &&
      item.errors.length === 0 &&
      promoted.formalIds.includes('2026-W30-legacy-import') &&
      promoted.legacyMirror &&
      promoted.currentText.includes('2026-W30-legacy-import') &&
      promoted.currentText.includes('NEW W30 REVIEW') &&
      promoted.history &&
      promoted.confidence === 70 &&
      imported.formal &&
      imported.legacyMirror &&
      imported.currentText.includes('2026-W31-quick-import') &&
      imported.currentText.includes('LATEST W31 REVIEW') &&
      imported.detailText.includes('2026-W30-legacy-import') &&
      imported.history &&
      imported.confidence === 82;
    await context.close();
  } catch (error) {
