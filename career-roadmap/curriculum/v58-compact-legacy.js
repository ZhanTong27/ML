(function(){
 function compactV58(){
  ...
  if(today&&!today.dataset.v58Compact){
   ...
   document.getElementById('openGrowthTopicV58')?.addEventListener('click',()=>{switchView('growth');S.growthHubV58.activeView='topics';save();mountGrowthHubV58()})
  }
  const portfolio=document.getElementById('uvmmsPortfolioV57');
  if(portfolio&&!portfolio.dataset.v58Compact){
   ...
   document.getElementById('openGrowthEvidenceV58')?.addEventListener('click',()=>{switchView('growth');S.growthHubV58.activeView='evidence';save();mountGrowthHubV58()})
  }
 }
 function install(){
  compactV58();
 ...
 window.addEventListener('load',()=>{[0,400,1200].forEach(x=>setTimeout(install,x))},{once:true});
